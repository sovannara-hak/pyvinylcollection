...
def updateScrobbleQueue(conn, trackInfo, bool):
	...
	if bool == "true":
		value = 1
	cursor.execute(query, (value, trackInfo.artist, trackInfo.title))
	print value
	conn.commit()
...
def insertIntoScrobbleQueue(conn, trackInfo):
	cursor = conn.cursor()
	query = """INSERT INTO ScrobbleQueue(Scrobble, Title, Artist, Length)
			VALUES (?, ?, ?, ?);""" 
	cursor.execute(query, (1, trackInfo.title, trackInfo.artist, trackInfo.duration))
	conn.commit()
...
def submitScrobble(conn):
	...
	cursor = conn.cursor()
	query = """SELECT Artist, Title, Length FROM ScrobbleQueue WHERE Scrobble = 1 ORDER BY Id;"""
	cursor.execute(query)
	for row in cursor:
                artist = row[0]
                title = row[1]
